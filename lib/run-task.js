...
/**
 * @typedef {'auto' | 'none' | '16' | '256'} ColorMode
 ...
 * @property {ColorMode} [colorMode] - Override terminal color capability detection.
 *
...
/** Named colors for terminals with 16-color support. */
const COLORS_NAMED = /** @type {const} */ ([
  'cyan', 'green', 'magenta', 'yellow', 'red', 'blue',
  'cyanBright', 'greenBright', 'magentaBright', 'yellowBright', 'redBright', 'blueBright',
  'white', 'gray',
])

/** ANSI 256-color codes for terminals with 256+ color support. */
const COLORS_256 = /** @type {const} */ ([
  51, 82, 196, 201, 226, 46, 45, 208, 93, 154,
  21, 213, 48, 165, 220, 33, 129, 214, 63, 202,
])

let colorIndex = 0
/** @type {Map<string, number>} */
const taskNamesToColorIndex = new Map()

/**
 * Returns a consistent color index for a task name, assigning one on first use.
 *
 * @param {string} taskName - The task name.
 * @returns {number} Color index.
 */
function getColorIndex (taskName) {
  let idx = taskNamesToColorIndex.get(taskName)
  if (idx === undefined) {
    idx = colorIndex++
    taskNamesToColorIndex.set(taskName, idx)
  }
  return idx
}

/**
 * Returns an `{ open, close }` ANSI color pair using the combined 256-color palette.
 * Named colors are used first, then the 256-color codes, for 34 distinct colors before wrapping.
 *
 * @param {number} idx - Color index.
 * @returns {{ open: string, close: string }} ANSI escape sequences.
 */
function selectColor256 (idx) {
  const total = COLORS_NAMED.length + COLORS_256.length
  const wrapped = idx % total
  if (wrapped < COLORS_NAMED.length) {
    const name = COLORS_NAMED[wrapped] ?? 'cyan'
    return /** @type {{ open: string, close: string }} */ (ansiStyles[name])
  }
  const code = COLORS_256[wrapped - COLORS_NAMED.length] ?? COLORS_256[0]
  return { open: ansiStyles.color.ansi256(code), close: ansiStyles.color.close }
}

/**
 * Returns an `{ open, close }` ANSI color pair for the given task name.
 *
 * @param {Writable} stream - The output stream (used to detect color depth when mode is 'auto').
 * @param {string} taskName - The task name.
 * @param {ColorMode} [colorMode] - Override color palette selection.
 * @returns {{ open: string, close: string }} ANSI escape sequences.
 */
function selectColor (stream, taskName, colorMode = 'auto') {
  const idx = getColorIndex(taskName)
  ...
  if (colorMode === '256') {
    return selectColor256(idx)
  }

  if (colorMode === '16') {
    const name = COLORS_NAMED[idx % COLORS_NAMED.length] ?? 'cyan'
    return /** @type {{ open: string, close: string }} */ (ansiStyles[name])
  }

  // auto: detect from stream
  const depth = /** @type {{ getColorDepth?: () => number }} */ (stream).getColorDepth?.() ?? 4
 ...
 */
function wrapLabeling (taskName, source, labelState, colorMode) {
  ...
  const color = isTTY ? selectColor(source, taskName, colorMode) : { open: '', close: '' }
...
export default function runTask (task, options) {
  ...
  async function asyncRunTask () {
    ...
    const stdout = wrapLabeling(task, options.stdout, options.labelState, options.colorMode)
    const stderr = wrapLabeling(task, options.stderr, options.labelState, options.colorMode)
