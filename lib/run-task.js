...
/**
 * Returns an `{ open, close }` ANSI color pair scaled to the color depth
 * supported by the given stream.
 *
 * @param {Writable} stream - The output stream (used to detect color depth).
 * @param {string} taskName - The task name.
 * @returns {{ open: string, close: string }} ANSI escape sequences.
 */
function selectColor (stream, taskName) {
...
function wrapLabeling (taskName, source, labelState) {
  ...
  const color = isTTY ? selectColor(source, taskName) : { open: '', close: '' }
